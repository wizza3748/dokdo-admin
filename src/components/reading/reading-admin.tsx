...
function WorkbookIcons({ book, online = false }: { book: ReadingBookRecord; online?: boolean }) {
  ...
  return <div className="flex max-w-[92px] flex-wrap justify-center gap-1">
    {Array.from({ length: count }, (_, index) => {
      ...
      if (online && active) return <Link key={index} title="온라인 독후감 설정" aria-label={`${book.title} 온라인 독후감 설정`} href={`/admin/exploration/reading/${book.id}/workbook/${getWorkbookRoundSetting(book.id).settingId}`} className="cursor-pointer text-blue-600 transition hover:scale-110 hover:text-blue-700">{icon}</Link>
