...
export function StudentHeader({ section }: { section?: string }) {
  ...
  return (
    <header className="relative z-30 flex h-12 items-center justify-between border-b border-[#dce4e8] bg-white px-6">
      ...
      <div className="absolute left-1/2 -translate-x-1/2 text-sm">{section ? <strong>{section}</strong> : <><span className="text-[#454c51]">{todayLabel}</span><strong className="ml-2">탐험</strong></>}</div>
