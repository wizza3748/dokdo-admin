...
import { AGENCY_ONLINE_WORKBOOKS, type FeedbackStatus, type OnlineWorkbook } from "@/lib/online-workbooks"
...
export function AgencyWorkbookList() {
  ...
  const [status, setStatus] = React.useState("작성전")
  ...
  const runBulkAction = (action: "ai" | "send" | "parent" | "flower") => {
    ...
    setNotice(`${selectedIds.length}건의 ${labels[action]} 작업을 목업 데이터에 반영했습니다.`)
    setSelectedIds([])
  }

  const labelClass = "w-[88px] shrink-0 text-right text-sm font-semibold text-slate-700"
  const bulkClass = "h-9 gap-2 border-slate-200 bg-white px-4 text-[13px] font-normal text-slate-400 shadow-none"
    ...
    <div className="w-full space-y-4 pb-2 text-slate-700">
      ...
      {filtersOpen && (
        <section className="grid gap-x-10 gap-y-3 bg-white px-8 py-6 lg:grid-cols-3">
          ...
          <div className="flex items-center gap-3"><span className={labelClass}>섬초롱꽃</span><CompactSelect value={flower} onValueChange={setFlower} placeholder="선택" options={[{ value: "all", label: "선택" }, { value: "yes", label: "지급" }, { value: "no", label: "미지급" }]} /></div>
      ...
      <section className="min-h-[700px] bg-white px-3 pt-4">
        <div className="flex flex-wrap items-center justify-between gap-3 pb-3">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            <Button variant="outline" disabled={!selectedIds.length} onClick={() => runBulkAction("ai")} className={bulkClass}><Bot className="size-4" />일괄 AI 피드백 생성 ({selectedIds.length})</Button>
            <Button variant="outline" disabled={!selectedIds.length} onClick={() => runBulkAction("send")} className={bulkClass}><Send className="size-4" />일괄 피드백 전송</Button>
            <Button variant="outline" disabled={!selectedIds.length} onClick={() => runBulkAction("parent")} className={bulkClass}><Mail className="size-4" />일괄 학부모 발송</Button>
            <Button variant="outline" disabled={!selectedIds.length} onClick={() => runBulkAction("flower")} className={bulkClass}><Flower2 className="size-4" />일괄 섬초롱꽃 지급 ({selectedIds.length})</Button>
            ...
            <TableBody>
              {filteredRecords.map((record) => (
                <TableRow key={record.id} className="h-[46px] text-slate-700 hover:bg-slate-50" data-state={selectedIds.includes(record.id) ? "selected" : undefined}>
                  ...
                  <TableCell className="text-center">{record.status === "전송완료" ? <Check className="mx-auto size-4 text-emerald-500" /> : <Send className="mx-auto size-4 text-slate-300" />}</TableCell>
                  <TableCell className="text-center">{record.parentSent ? <Check className="mx-auto size-4 text-emerald-500" /> : <Mail className="mx-auto size-4 text-slate-300" />}</TableCell>
