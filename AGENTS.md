...
- 기존 디자인과 레이아웃은 임의로 변경하지 않는다. 기존 화면 구조·컴포넌트·스타일을 최대한 그대로 재사용한다.
- 디자인 또는 레이아웃 변경이 필요하면, 명세에 포함된 변경이라도 이유와 범위를 설명하고 사용자 승인을 먼저 받는다.
- 기능 추가 요청은 임의의 화면 재설계 승인으로 해석하지 않는다.
- 사용자가 요청한 디자인 원복은 해당 임의 변경에 한정하며 기존 데이터와 기능, 관련 없는 변경은 보존한다.
